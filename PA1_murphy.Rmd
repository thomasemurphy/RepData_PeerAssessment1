...
The mean number of steps per day is `r format(mean_steps, digits=5, scientific=FALSE)`.
...
The interval with the largest number of steps, averaged over all the days of the study, is Interval `r most_active_interval`.
...
It looks like about `r format(frac_missing * 100, digits=4)`% of values in the "steps" column are NA. I verified that there are no NA values in the "date" or "interval" columns.
...
The mean number of steps per day is `r format(mean_steps_imputed, digits=5, scientific=FALSE)`, and the median number of steps per day is exactly equal to the mean, at `r format(median_steps_imputed, digits=5, scientific=FALSE)`.
